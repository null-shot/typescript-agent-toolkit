...
import { WifiOff, Plus, Circle, Check } from "lucide-react";
...
export function AgentSelector({
  ...
  const refreshAgentsRef = React.useRef(refreshAgents);
  ...
  const handleAgentAdded = () => {
    // The context already handles adding and auto-selecting the new agent
  ...
  return (
    <div className={cn("flex items-center gap-3", className)}>
      ...
      <Select value={currentAgent?.id} onValueChange={handleValueChange}>
        <SelectTrigger className="w-[200px] bg-[#1e1e1e] border-[rgba(255,255,255,0.1)] text-white shadow-sm">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            {currentAgent && currentAgent.health?.isOnline && (
              <Circle
                className="h-2 w-2 text-[#00d96f] shrink-0"
                fill="currentColor"
              />
            )}
        ...
        <SelectContent className="bg-[#252525] border-[rgba(255,255,255,0.08)]">
          {agents.map((agent) => (
            <SelectPrimitive.Item
              ...
              className={cn(
                "cursor-pointer hover:bg-[#303030] focus:bg-[#303030] py-3 relative flex w-full items-center gap-2 rounded-sm pr-8 pl-2 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
          ...
          <SelectItem
            value="__add_agent__"
            className="cursor-pointer hover:bg-[rgba(0,212,170,0.1)] focus:bg-[rgba(0,212,170,0.1)] border-t border-[rgba(255,255,255,0.08)] mt-1"
          >
