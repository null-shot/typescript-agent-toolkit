...
export abstract class NullShotAgent<ENV extends AgentEnv, MESSAGE extends any = any> implements DurableObject {
	...
	protected async initializeServices(): Promise<void> {
		...
		console.log(`🔧 NullShotAgent.initializeServices: Starting initialization of ${this.services.length} services`)
		for (const service of this.services) {
			console.log(`🔍 NullShotAgent.initializeServices: Processing service "${service.name || 'unnamed'}"`)
			if (service.initialize) {
				console.log(`⏳ NullShotAgent.initializeServices: Calling initialize() for "${service.name}"`)
				try {
					await service.initialize();
					console.log(`✅ NullShotAgent.initializeServices: Service "${service.name}" initialized successfully`)
				} catch (error) {
					console.error(`❌ NullShotAgent.initializeServices: Failed to initialize "${service.name}":`, error)
					throw error
				}
			} else {
				console.log(`⏭️  NullShotAgent.initializeServices: Service "${service.name}" has no initialize() method`)
			}

			// Register routes for external services
			if (isExternalService(service)) {
				console.log(`🔗 NullShotAgent.initializeServices: Registering routes for external service "${service.name}"`)
				service.registerRoutes(this.app);
			}
		}

		// Register default /mcp and /tools endpoints that return empty data.
		// These are registered AFTER service routes, so if ToolboxService already
		// registered its own /mcp handler, Hono dispatches to that first (first-match wins).
		// For agents without ToolboxService, these defaults ensure a clean empty response
		// instead of a 404, which the playground UI expects.
		try {
			this.app.get('/mcp', (c) => c.json({ mcpServers: [] }, 200));
			this.app.get('/tools', (c) => c.json({ tools: [] }, 200));
		} catch (error) {
			// Hono matcher may already be built if a request arrived before initialization completed
			if (error instanceof Error && error.message.includes('matcher is already built')) {
				console.warn('⚠️ NullShotAgent: Could not register default /mcp and /tools routes (matcher already built)');
			} else {
				throw error;
			}
		}

		console.log(`✅ NullShotAgent.initializeServices: All services initialized`)
	}
	...
	protected setupRoutes(app: Hono<{ Bindings: ENV }>) {
		...
		app.post('/agent/chat/:sessionId', async (c) => {
			try {
				console.log('🚨🚨🚨 [Agent] POST /agent/chat/:sessionId ROUTE CALLED!');
