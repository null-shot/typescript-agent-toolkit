...
export function parseCloudflareError(rawError: string): ErrorInfo {
	...
	return {
		...
		message: firstLine.replace(/✘ \[ERROR\]/, "").trim() || "Unknown error",
		code: "UNKNOWN"
	}
}
...
export function formatErrorCard(error: ErrorInfo, _component: string): string {
	...
	lines.push(chalk.red(`│`) + chalk.red.bold(` ✖ ${error.title}`.padEnd(48)) + chalk.red(`│`))
	lines.push(chalk.red(`├${"─".repeat(48)}┤`))
	
	// Message
	const msgLines = wrapText(error.message, 46)
	for (const line of msgLines) {
		lines.push(chalk.red(`│`) + chalk.white(` ${line}`.padEnd(48)) + chalk.red(`│`))
	}
	
	// Hint (if present)
	if (error.hint) {
		lines.push(chalk.red(`│`) + " ".repeat(48) + chalk.red(`│`))
		lines.push(chalk.red(`│`) + chalk.cyan(` 💡 ${error.hint}`.padEnd(48)) + chalk.red(`│`))
	}
	
	// Link (if present)
	if (error.link) {
		lines.push(chalk.red(`│`) + chalk.gray(` 🔗 ${error.link}`.substring(0, 48).padEnd(48)) + chalk.red(`│`))
	}
...
export function formatWarningCard(title: string, message: string, hint?: string): string {
	...
	lines.push(chalk.yellow(`│`) + chalk.yellow.bold(` ⚠ ${title}`.padEnd(48)) + chalk.yellow(`│`))
	lines.push(chalk.yellow(`├${"─".repeat(48)}┤`))
	
	const msgLines = wrapText(message, 46)
	for (const line of msgLines) {
		lines.push(chalk.yellow(`│`) + chalk.white(` ${line}`.padEnd(48)) + chalk.yellow(`│`))
	}
	
	if (hint) {
		lines.push(chalk.yellow(`│`) + " ".repeat(48) + chalk.yellow(`│`))
		lines.push(chalk.yellow(`│`) + chalk.cyan(` 💡 ${hint}`.padEnd(48)) + chalk.yellow(`│`))
	}
