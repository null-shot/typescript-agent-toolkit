...
export function setupAgentHandlers(bot: Bot, env: Env): void {
	...
	bot.callbackQuery(/^select_agent:(.+)$/, async (ctx) => {
		...
		try {
			...
			log.debug("Could not edit message", { error })
		}
...
/**
 * Get all agent info for a chat in a single KV read.
 * Combines getCurrentAgentUrl + getCurrentAgentName + getCurrentAgentId
 * to avoid 3 redundant KV reads for the same session data.
 */
export async function getCurrentAgentInfo(
	env: Env,
	chatId: string
): Promise<{ url: string; name: string; id: string; sessionId: string }> {
	const sessionData = await getOrCreateSessionData(env.SESSIONS, chatId)
	const agents = getAvailableAgents(env)
	const defaultAgents = getDefaultAgents(env.AGENT_URL)

	const url = sessionData.selectedAgentUrl || env.AGENT_URL

	let name = defaultAgents[0]?.name || "AI Agent"
	let id = defaultAgents[0]?.id || "default-agent"

	if (sessionData.selectedAgentId) {
		id = sessionData.selectedAgentId
		const agent = findAgentById(agents, sessionData.selectedAgentId)
		if (agent) {
			name = agent.name
		}
	...
	return { url, name, id, sessionId: sessionData.sessionId }
}
